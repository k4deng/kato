...
function getTotalTime(timeFormat) {
	...
	if (!timeFormat.endsWith('d') && !timeFormat.endsWith('h') && !timeFormat.endsWith('m') && !timeFormat.endsWith('s')) {
		return { error: 'Please use one of the following time delimiters: `d`,  `h`,  `m`, `s`.' };
	}
	// make sure its a number infront of the time delimiter
	if (isNaN(timeFormat.slice(0, -1))) return { error: 'An invalid number was entered. (Example: 1m or 16h)' };

	// convert timeFormat to milliseconds
	const time = require('ms')(timeFormat);

	// Make sure time isn't over 10 days
	if (time >= 864000000) return { error: "Can't be longer than 10 days." };
