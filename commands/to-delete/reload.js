...
exports.run = async (client, message, args, level) => { // eslint-disable-line no-unused-vars
  ...
  const command = container.commands.get(args[1]) || container.commands.get(container.aliases.get(args[1]));
  // Check if the command exists and is valid
  if (!command) {
    return message.reply("That command does not exist");
  }
  // the path is relative to the *current folder*, so just ./filename.js
  delete require.cache[require.resolve(`../${args[0]}/${command.help.name}.js`)];
  ...
  const props = require(`../${args[0]}/${command.help.name}.js`);
...
exports.help = {
  ...
  usage: "reload [subfolder] [command]"
};